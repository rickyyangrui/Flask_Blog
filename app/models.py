...
class Menu(db.Model):
    ...
    @staticmethod
    def insert_menus():
        menus = [u'Web开发', u'数据库', u'知乎', u'随笔', u'Git', u'福利网址'
                 u'Linux', u'开发语言']
        for name in menus:
            menu = Menu(name=name)
            db.session.add(menu)
...
class ArticleType(db.Model):
    ...
    def insert_articleTypes():
        articleTypes = ['Python', 'Java', 'JavaScript', 'Django',
                        'CentOS', 'Ubuntu', 'MySQL', 'Oracle', 'Flask'
                        u'Linux命令大全', u'git命令', u'编程人生',
                        u'XXX是一种什么样的体验', u'福利网址',
                         ]
...
class BlogInfo(db.Model):
    ...
    def insert_blog_info():
        blog_mini_info = BlogInfo(title=u'小青蛙',
                                  signature=u'自律给我自由',
                                  navbar='inverse')
